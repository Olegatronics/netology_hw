...
2. Убедитесь что у вас УСТАНОВЛЕН ```docker compose```(без тире) версии не менее v2.24.X, для это выполните команду ```docker compose version```  
    ...
    Установил Docker compose отдельно командой
    ```bash
...
1. Сделайте в своем github пространстве fork [репозитория](https://github.com/netology-code/shvirtd-example-python/blob/main/README.md).
   ...
   [**ответ**] Создал форк. Выбрал необходимый репозиторий и наджал на иконку ```fork```, после чего форк скопировался в мой репозиторий.
...
Обязательно используйте конструкцию ```COPY . .``` в Dockerfile. Не забудьте исключить ненужные в имадже файлы с помощью dockerignore. Протестируйте корректность сборки.  
    ...
    [**ответ**] Создал ```Dockerfile.python``` для сборки проекта, в качестве базового образа добавил ```python:3.9-slim```.
    ...
    Проверяем командой ```docker images``` создался ли наш образ  
